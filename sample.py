...
# Ensure imports resolve to model/ modules
...
def _build_config(args, logdir: Path):
    ...
    model_name = str(args.model).replace(".yaml", "")
    ...
    overrides = omegaconf.OmegaConf.create({
        ...
        "eval": {"checkpoint_path": str(_resolve_user_path(args.ckpt))},
        "data": {"tokenizer_name_or_path": str(_resolve_user_path(args.vocab))},
    })
...
def main() -> int:
    ...
    parser.add_argument("-c", "--ckpt", type=str, default="weights/50M-epoch9-1145000.ckpt")
    parser.add_argument("-m", "--model", type=str, default="small-50M")
    parser.add_argument("-T", "--steps", type=int, default=300)
    parser.add_argument("-p", "--nucleus", type=float, default=1)
    ...
    parser.add_argument("-v", "--vocab", type=str, default="model/vocab_V2.txt")
    ...
    parser.add_argument("--temperature", type=float, default=1.1, help="Sampling temperature (default 1.0)")
