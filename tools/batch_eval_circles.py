...
DEFAULT_JOBS = 20
CIRCLE_PATTERN = re.compile(r"#Circles \(hit set\):\s*(\d+)")
...
def default_output_path(input_dir: Path) -> Path:
    ...
    return input_dir.parent / f"batch_circles_{input_dir.name}_{stamp}.txt"


def default_target_stats_output_path(output_path: Path) -> Path:
    return output_path.with_name(f"{output_path.stem}_target_circles_stats.txt")
...
def extract_circle_value(stdout: str) -> float | None:
    match = CIRCLE_PATTERN.search(stdout)
    if match is None:
        return None
...
def run_eval(
    ...
    if proc.stdout:
        ...
        circle_value = extract_circle_value(proc.stdout)
    if proc.returncode == 0:
        ...
        status = "fail"
    lines.append("")
    return index, status, "\n".join(lines), target, circle_value
...
def main() -> int:
    ...
    eval_script = str(Path(__file__).with_name("eval_circles.py").resolve())
    ...
    target_circles_values: dict[str, list[float]] = defaultdict(list)
    ...
    if tasks:
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                index, status, block, target, circle_value = future.result()
                blocks[index] = block
                if status == "success":
                    success += 1
                    if circle_value is not None:
                        target_circles_values[target].append(circle_value)
                else:
                    fail += 1
    else:
        workers = 0
    ...
    with output_path.open("w", encoding="utf-8") as out:
        print(f"Batch circles eval start: {start_time.strftime('%a %b %d %H:%M:%S %Z %Y')}", file=out)
        ...
        print(
            f"Batch circles eval end: {datetime.now().astimezone().strftime('%a %b %d %H:%M:%S %Z %Y')}",
            file=out,
        )

    with target_stats_output_path.open("w", encoding="utf-8") as out:
        print(f"Target circles stats from: {output_path}", file=out)
        print(
        ...
        for target in TARGET_NAMES:
            scores = target_circles_values.get(target, [])
            if not scores:
                print(f"{target}\tn=0\tmean_circles=NA\tstd_circles=NA", file=out)
                continue
            mean_val = statistics.mean(scores)
            std_val = statistics.stdev(scores) if len(scores) >= 2 else 0.0
            values = ",".join(f"{score:.3f}" for score in sorted(scores))
            print(
                f"{target}\tn={len(scores)}\tmean_circles={mean_val:.3f}\tstd_circles={std_val:.3f}\tvalues={values}",
                file=out,
            )
